<?php
gp_title(__('Create New Translation Set', 'glotpress'));
$project ? gp_breadcrumb_project($project) : gp_breadcrumb(array(__('New Translation Set', 'glotpress')));
gp_tmpl_header();
?>
<h2><?php _e( 'Create New Translation Set' , 'glotpress'); ?></h2>
<form action="" method="post">
<?php gp_tmpl_load( 'translation-set-form', get_defined_vars()); ?>

	<p>
		<label for="set[recursive]"><?php _e('Recursively create sets in subprojects', 'glotpress'); ?></label>
		<input type="checkbox" id="set[recursive]" name="set[recursive]" <?php gp_checked(gp_get_option('default_recursive_sets') == 'on'); ?> /></p>

	<p>
		<input type="submit" name="submit" value="<?php echo esc_attr( __('Create', 'glotpress') ); ?>" id="submit" />
		<span class="or-cancel">or <a href="javascript:history.back();">Cancel</a></span>
	</p>
</form>
<?php gp_tmpl_footer();