<?php
gp_title(__('Elgg Import', 'glotpress'));
gp_tmpl_header();
...
<dl>
	<dt><h3><?php _e('Elgg Package Import', 'glotpress'); ?></h3></dt>
	<dd>
		...
		<p>
		<label for="import[core_project]"><?php _e('Project to import Elgg cores into', 'glotpress'); ?></label><br/>
		<?php echo $project_dropdown1; ?>
		</p>
		<p>
		<label for="import[plugin_project]"><?php _e('Project to import third party plugins into', 'glotpress'); ?></label><br/>
		<?php echo $project_dropdown2; ?>
		</p>
		<?php // <p> Either </p> ?>
		<p>
		<?php // <input type="radio" value="zip" name="import[elggtype]" id="elggtypezip"> ?>
		<label for="upload"><?php _e('Select the Elgg install zip file', 'glotpress'); ?></label><br/>
		<input type="file" name="upload" id="elggfile"><br/>
		<small><?php _e('You can get this file from <a href="http://www.elgg.org/download.php" target="_blank">the Elgg web site</a>. Make sure the file is in ZIP format.', 'glotpress'); ?></small>
		</p>
		<?php /*
		<p> Or </p>
		<p>
		...
		<label for="import[originals]"><?php _e('Also import original English texts', 'glotpress'); ?></label><br/>
		</p>
		<p>
		<input type="checkbox" name="import[overwrite]" value="on">
		<label for="import[overwrite]"><?php _e('Do not overwrite exisiting translation sets', 'glotpress'); ?></label><br/>
		</p>
	</dd>
</dl>
<p>
	<input type="submit" name="submit" value="<?php echo esc_attr(__('Import', 'glotpress')); ?>" id="submit" />
</p>
</form>
<?php
/*
