...
<h2><?php echo wptexturize(sprintf(__('Installation Process (phase %d of %d)', 'glotpress'), 2, GP_TOT_INSTALL_PAGES)); ?></h2>
<form action="" method="post">

<dl>
	<dt><h3><?php echo __('Configure Administrator Options', 'glotpress'); ?></h3></dt>
	<dd>
<?php if ( defined('CUSTOM_USER_TABLE') ) { ?>
		...
		<small><?php _e('Select the WordPress user that will have administrator\'s rights in GlotPress', 'glotpress'); ?></small>
<?php } else { ?>
		<label for="config[gp_admin_username]"><?php echo __('Admin Username', 'glotpress'); ?></label>
		<input type="text" name="config[gp_admin_username]" value="<?php echo $config['gp_admin_username']; ?>" id="config[gp_admin_username]"><br/>
		<label for="config[gp_admin_name]"><?php echo __('Display Name', 'glotpress'); ?></label>
		...
		<label for="config[gp_admin_password]"><?php echo __('Admin Password', 'glotpress'); ?></label>
		<input type="password" name="config[gp_admin_password]" value="<?php echo $config['gp_admin_password']; ?>" id="config[gp_admin_password]"><br/>
		<label for="config[gp_admin_password2]"><?php echo __('Confirm Password', 'glotpress'); ?></label>
		...
		<label for="config[gp_admin_email]"><?php echo __('Email Address', 'glotpress'); ?></label>
		<input type="text" name="config[gp_admin_email]" value="<?php echo $config['gp_admin_email']; ?>" id="config[gp_admin_email]"><br/>
		<small><?php _e('The default username is "admin", with a password of "a".', 'glotpress'); ?></small><br/>
<?php } ?>
	</dd>
</dl>
...
<?php echo gp_js_focus_on('config[GPDB_NAME]'); ?>
	...
	<p>
		<input type="submit" name="submit" value="<?php echo esc_attr(__('Finish', 'glotpress')); ?>" id="submit" />
	</p>
</form>
